...
public class HbaseMonitoringEventProducer implements EventProducer , Runnable { 

	private final	List<HbaseTriggerMonitoredSourceInfo> _sources ;
	...
	public HbaseMonitoringEventProducer(String name,String dbname, String uri,List<HbaseTriggerMonitoredSourceInfo> sources,MBeanServer mbeanServer) {
		_log.info("HbaseMonitoringEven is starting...");
		_sources = sources;
		_name = name;
		...
		for(HbaseTriggerMonitoredSourceInfo sourceInfo : sources)
		{
		  if (null==_schema)
		  {
	...
	protected boolean createHTable() { //create HBaseTable to store max_scn for all hbase table.
		try {
	...
	@Override
	public void run() {  //get the max_scn of all tables and max_scn of each tables store them in dbstats...
				...
				try {
				  ...
				  _dbStats.setMaxDBScn(maxDBScn);
					for (HbaseTriggerMonitoredSourceInfo source: _sources) {
						...
						if (rs.isEmpty() == false)
						{
							long maxScn = Bytes.toLong((rs.getValue(Bytes.toBytes("table"), Bytes.toBytes(source.getEventTable()))));
	 ...
	 private long getMaxlogSCN() //read the max_scn of all hbase table
	  {
		...
		try {
			...
			Result rs = hTable.get(get);
			maxScn = Long.parseLong(Bytes.toString(rs.getValue(Bytes.toBytes("max_scn"), Bytes.toBytes(""))));
